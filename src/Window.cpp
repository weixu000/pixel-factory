...
void Window::SetupCallbacks() {
  ...
  glfwSetMouseButtonCallback(
      window_, [](GLFWwindow *window, int button, int action, int mods) {
        ...
        switch (e.action) {
          case GLFW_PRESS:
            Retrieve(window)->handler_->ProcessEvent("MouseButtonPress", e);
            break;
          case GLFW_RELEASE:
            Retrieve(window)->handler_->ProcessEvent("MouseButtonRelease", e);
            break;
          default:
            break;
        }
  ...
  glfwSetCursorPosCallback(window_, [](GLFWwindow *window, double x, double y) {
    Retrieve(window)->handler_->ProcessEvent(
        "MouseMove", CursorPositionEvent{{}, float(x), float(y)});
  });
