...
namespace {
std::tuple<tinyobj::attrib_t, std::vector<tinyobj::shape_t>> LoadTinyObj(const std::string &file_path) {
...
std::tuple<std::vector<GLuint>, std::vector<Vertex>> LoadVerticesFromObjFile(const std::string &file_path) {
  ...
  for (auto &shape:shapes) {
    for (auto &index : shape.mesh.indices) {
      auto v = 3 * index.vertex_index;
      auto[it_index, inserted] = indices_map.insert(std::make_pair(v, attributes.size()));
      if (inserted) {
        attributes.emplace_back(
            glm::vec3(attrib.vertices[v], attrib.vertices[v + 1], attrib.vertices[v + 2])
        );
